...
<div class="layui-container" style="margin-top: 50px;">
    ...
    <script>
        layui.use('table', function(){
            ...
            table.render({
                elem: '#test'
                ,url:'bookAdmin.do?method=getBorrows'
                ,title: '借书列表'
                ,cols: [[
            ...
            table.on('tool(test)', function(obj){
                ...
                if(obj.event === 'agree'){
                    layer.confirm('确定通过？', function(index){
                        window.location.href="bookAdmin.do?method=agreeBorrow&id="+data.id;
                        layer.close(index);
                    });
                } else if(obj.event === 'disagree'){
                    layer.confirm('确定拒绝？', function(index){
                        window.location.href="bookAdmin.do?method=disAgreeBorrow&id="+data.id+"&bookId="+data.bookId;
                        layer.close(index);
                    });
                }
            });
